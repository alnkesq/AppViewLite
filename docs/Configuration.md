...
* `APPVIEWLITE_FIREHOSES`: A comma-separated list of ATProto or JetStream firehoses to listen to. Each entry can provide multiple fallback URLs (separated by `|`). Each fallback URL is in the format `jet:domain.example` (for JetStream), or `domain.example` (for ATProto firehoses). For example, `jet:jetstream1.us-east.bsky.network|jet:jetstream2.us-east.bsky.network`. Defaults to the JetStream firehoses (jetstream*.us-*.bsky.network). Only use firehoses you trust, since DAG-CBOR signatures are not verified. For out-of-network PDSes that are not listened to by the main firehoses, use instead `APPVIEWLITE_DID_DOC_OVERRIDES` (see below).
