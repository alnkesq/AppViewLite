...
namespace AppViewLite.Models
{
    ...
    {
        public readonly int CompareTo(PostId other)
        {
            var z = Author.CompareTo(other.Author);
            if (z != 0) return z;
            return PostRKey.CompareTo(other.PostRKey);
        }
        public readonly string Serialize() => ((PostIdTimeFirst)this).Serialize();
        public static PostId Deserialize(string s) => PostIdTimeFirst.Deserialize(s);

        public override readonly string ToString() => $"{nameof(PostId)}({Author.PlcValue}, {PostRKey})";
    }
}
